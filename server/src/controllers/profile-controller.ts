...
    // Logic to handle the photo URL: check for new file, then existing URL, then DB
    if (req.file && req.file.buffer) {
      console.log('File uploaded. Uploading to Cloudinary...');
      const result = await uploadToCloudinary(req.file.buffer, 'profile-images');
      photoUrl = result.url;
      console.log('Cloudinary URL:', photoUrl);
    } else if (req.body.photoUrl) {
      console.log('Using existing photo URL from request body.');
      photoUrl = req.body.photoUrl;
    } else {
      console.log('No new photo, checking database for existing URL.');
